...
void writeCXI(cEventData *info, cGlobal *global ){
	...
	DETECTOR_LOOP {
		...
		if(global->saveAssembled) {
			...
			if (cxi->entry.images[detID].thumbnail<0)
				{ERROR("No valid dataset.");}
		...
		if(global->saveRaw){
			if (cxi->entry.instrument.detectors[detID].data<0)
				{ERROR("No valid dataset.");}
