...
int hitfinder1(cGlobal *global, cEventData *eventData, long detID){
  ...
  uint16_t  pixel_options = PIXEL_IS_IN_PEAKMASK | PIXEL_IS_OUT_OF_RESOLUTION_LIMITS | PIXEL_IS_HOT | PIXEL_IS_BAD | PIXEL_IS_SATURATED;
...
int peakfinder5(cGlobal *global, cEventData *eventData, int detID) {
	...
	for(long i=1; i<global->detector[detID].asic_nx-1; i++){
		...
		if ( isBitOptionSet(mask[e],PIXEL_IS_OUT_OF_RESOLUTION_LIMITS) ) continue;
		...
		if ( global->hitfinderCheckGradient == 1 ){
			...
			if ( isBitOptionSet(mask[e],PIXEL_IS_BAD) ) continue;
			if ( isBitOptionSet(mask[e+1],PIXEL_IS_BAD) ) continue;
			if ( isBitOptionSet(mask[e-1],PIXEL_IS_BAD) ) continue;
			if ( isBitOptionSet(mask[e+global->detector[detID].pix_nx],PIXEL_IS_BAD) ) continue;
			if ( isBitOptionSet(mask[e-global->detector[detID].pix_nx],PIXEL_IS_BAD) ) continue;
		...
		if ( global->hitfinderSubtractLocalBG == 1 ) {
			...
			for ( lbg_fs = fsmin; lbg_fs <= fsmax; lbg_fs++ ) {
				...
				if ( isBitOptionUnset(mask[lbg_e],PIXEL_IS_BAD) ) {
		...
		badpix += isAnyOfBitOptionsSet(mask[e],combined_pixel_options);
		...
		do {
			...
			for(long p=0; p<nat; p++){
				...
				for(long k=0; k<search_n; k++){
					...
					badpix += isBitOptionSet(mask[e],PIXEL_IS_BAD);
