  ...
  , m_commonModeFile_1D("")
{
	...
	m_outputFluores = config("outputFluores");
  	...
  	m_lookAtSubregionSpectra = config("lookAtSubregionSpectra");
...
void chuck_ana_mod::event(Event& evt, Env& env)
{
	...
	if (m_outputFluores) {
		...
		if (elem1.get()) {
			...
			} else { // no common mode correction
				for (int i = sx; i < ex; i++) { // width
					for (int j = sy; j < ey; j++) { // height
						if (data[i][j][k] > 0 && data[i][j][k] < hotpixel) {
							sumPerShot += ( data[i][j][k] - darkSubregion[i-sx][j-sy] ) / gainRegion[i+k*dimX][j];
							goodPixPerShot++;
						}
					}
				}
	...
	if (m_generateCommonMode) {
		...
		if (elem1.get()) {
			...
			double sumPerShot = 0;
			int goodPixPerShot = 0;
			// common mode from dark patch of cspad2x2
	...
	if (m_lookAtSubregion && m_eventNumber == nevent) {
		stringstream sstm;
		sstm << "subregion_r" << runNumber << "_e" << nevent << ".txt";
	...
	if (m_lookAtSubregionSpectra && m_eventNumber == nevent) {
		stringstream sstm;
		sstm << "subregionSpectra_r" << runNumber << "_e" << nevent << ".txt";
		string result = sstm.str();
		...
		if (elem1.get()) {
			...
			for (int i = sx; i < ex; i++) { // width
				for (int j = sy; j < ey; j++) { // height
					outFile << ( data[i][j][k] - darkSubregion[i-sx][j-sy] - commonMode[nevent] ) / gainRegion[i+k*dimX][j] << " ";
				}
			}
		}
		outFile.close();
	...
	if (m_lookAtCsPad2x1 && m_eventNumber == nevent) {
		stringstream sstm;
		sstm << "cspad2x1_r" << runNumber << "_e" << nevent << ".txt";
		string result = sstm.str();
		ofstream outFile(result.c_str()); // modify myPsana1 to r117
...
void chuck_ana_mod::endRun(Event& evt, Env& env)
{
	...
	if (m_outputFluores) {
		...
		if (existCM) {
			...
			sstm << "r" << runNumber << "_cspad2x2_fluoresNoCM";
		}
		string result = sstm.str();
  		ofstream outFlu(result.c_str()); // output fluorescence for all shots
  		for (unsigned i = 0; i < fluores.size(); i++ ) {
	...
	if (m_generateCommonMode) {
		// Write out files
		stringstream sstm;
		sstm << "r" << runNumber << "_cspad2x2_commonMode";
		string result = sstm.str();
