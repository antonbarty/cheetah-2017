...
cGlobal::cGlobal(void) {
    ...
    hitfinderMinSNR = 40;
	strcpy(hitfinderHistogramFile, "No_file_specified");
	strcpy(hitfinderThresholdMapFile, "No_file_specified");
...
void cGlobal::setup() {
	...
	// Read hitfinder threshold map file (only needed for hitfinder 8)
	if(strcmp(hitfinderThresholdMapFile, "No_file_specified") != 0) {
		if(hitfinder == 8)
			readHitfinderThresholdMap(self);
	};
	
	
	// Read hitfinder histogram file (only needed for hitfinder 9)
	if(strcmp(hitfinderHistogramFile, "No_file_specified") != 0) {
		if(hitfinder == 9)
			readHitfinderHistogram(self);
...
int cGlobal::parseConfigTag(char *tag, char *value) {
	...
	else if(!strcmp(tag,"hitfinderhistogramfile")){
		strcpy(hitfinderHistogramFile, value);
	}
	else if(!strcmp(tag,"hitfinderthresholdmap")){
		strcpy(hitfinderThresholdMapFile, value);
	}
