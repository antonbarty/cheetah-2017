...
//	cout << "OK" << endl;
	...
	eventData->laserEventCodeOn = 0;
    	...
    	eventData->gmd1 = gmd1;
    	eventData->gmd2 = gmd2;
	eventData->gmd11 = fee->f_11_ENRC();
	eventData->gmd12 = fee->f_12_ENRC();
	eventData->gmd21 = fee->f_21_ENRC();
	eventData->gmd22 = fee->f_22_ENRC();
	...
	for(long detID=0; detID<cheetahGlobal.nDetectors; detID++) {
		...
		cout << "detectorZ: " << eventData->detector[detID].detectorZ << endl;
    	}
...
//        memcpy(eventData->pulnixImage, pulnixImage, (long)pulnixWidth*(long)pulnixHeight*sizeof(unsigned short));
	...
	cout << "nprocessedframes: " << cheetahGlobal.nprocessedframes << endl;
*/
}
