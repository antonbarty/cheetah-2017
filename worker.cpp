					...
					for(long j=1; j<CSPAD_ASIC_NY-1; j++){
						for(long i=1; i<CSPAD_ASIC_NX-1; i++){
							...
							// This might be the start of a peak - start searching
								...
								lbg_counter = 0;
								if ( global->hitfinderSubtractLocalBG ) {
...
void saveRunningSums(cGlobal *global) {
	 ...
	 */		
	double *buffer;
	if((global->powderSumHits || global->generateGaincal || global->generateDarkcal) && global->saveAssembled) {
		// Hits
	...
	if(global->powderSumBlanks && global->saveAssembled) {
		// Blanks
