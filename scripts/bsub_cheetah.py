...
def print_table_terminal():
    ...
    print ("-"*term_shape[1])
    print ("Run\ttype\tstatus\t#frames\t#hits\thrate")
...
def write_table_google():
    write_row_google(0,["Run","Type","Status","#Frames","#Hits","Hit rate [%]"])
    rs = runs_info.keys()
    rs.sort()
    for i,r in zip(range(len(rs)),rs):
        d = runs_info[rs[i]]
        write_row_google(i+1,[rs[i],d["type"],d["status"],d.get("#frames",""),d.get("#hits",""),d.get("hrate","")])
...
def read_row_google(row):
    cells = []
    col = 0
    while True:
	c = read_cell_google(row,col)
	if c != None:
	    cells.append(c)
	    col += 1
	else:
...
def read_col_google(col):
	...
	if c != None:
	    ...
	    break
    return cells
def read_cell_google(row,col):
    s = spr_client.GetCellsFeed(key=spreadsheet_key, wksht_id=worksheet_id,cell="R%iC%i" % (row+1,col+1)).content.text
    #print s
    return s
